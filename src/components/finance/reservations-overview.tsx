...
// ===========================================================================
// Visão de Reservas & Pagamentos — conteúdo server-rendered pré-Fase 5,
// preservado como aba do FinanceShell (receita de reservas, canais, imóveis,
// pagamentos/Asaas). Filtro de período recarrega a página mantendo a aba.
// ===========================================================================
...
export async function ReservationsOverview({ companyId, range }: ReservationsOverviewProps) {
  const [summary, paymentsRaw, openChargesRaw] = await Promise.all([
    getFinanceSummary(companyId, range.from, range.to),
    listPaymentsForCompany(companyId, { from: range.from, to: range.to }),
    ...
    <>
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-xs text-gray-500">
          {formatDate(range.from)} → {formatDate(range.to)} · receita de reservas e pagamentos
        </p>
