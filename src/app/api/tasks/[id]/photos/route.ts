...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const path = `housekeeping/${id}/${kind}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
    ...
    const { data: row, error: insErr } = await (supabase.from("task_photos") as any) // eslint-disable-line @typescript-eslint/no-explicit-any
      .insert({ company_id: task.company_id, task_id: id, kind, url: publicUrl, uploaded_by: user.id })
      .select("id, kind, url, created_at, uploaded_by")
      .single();
...
/** Remove a task photo. Body: { photo_id }. */
export async function DELETE(request: NextRequest, { params }: Params) {
  try {
    ...
    const { data: photo } = await supabase
      ...
      .eq("id", photoId)
      .maybeSingle();
    const p = photo as { id: string; company_id: string; task_id: string } | null;
